...
test("lists the caller's own mailboxes, and never their signing secret", async () => {
	const { auth } = await harness();
	const { headers } = await signIn(auth, "ada@example.com");
	await auth.api.mailkiteProvisionMailbox({ body: { localPart: "ada" }, headers });

	const res: any = await auth.api.mailkiteListMailboxes({ headers });
	assert.equal(res.mailboxes.length, 1);
	assert.equal(res.mailboxes[0].address, "ada@acme.com");
	// This response reaches the browser; the route secret is a credential.
	assert.equal(
		JSON.stringify(res).includes("signingSecret"),
		false,
		"the signing secret must never leave the server",
	);
});

test("one user cannot see another's mailboxes", async () => {
	const { auth } = await harness();
	...
	const res: any = await auth.api.mailkiteListMailboxes({ headers: bob.headers });
	assert.deepEqual(res.mailboxes, []);
});
...
test("a reply is stored on the same thread, so the conversation has both halves", async () => {
	const { auth } = await harness();
	const { headers } = await signIn(auth, "ada@example.com");
	await auth.api.mailkiteProvisionMailbox({ body: { localPart: "ada" }, headers });
	await deliver(auth, {
	...
	const before: any = await auth.api.mailkiteListMessages({ headers });
	const original = before.messages[0];

	await auth.api.mailkiteReply({
		body: { messageId: original.id, text: "answer" },
		headers,
	});

	const after: any = await auth.api.mailkiteListMessages({ headers });
	assert.equal(after.messages.length, 2, "the sent reply must be persisted, not just sent");

	const outbound = after.messages.find((m: any) => m.direction === "outbound");
	assert.ok(outbound, "the reply should be marked outbound");
	assert.equal(outbound.fromAddress, "ada@acme.com");
	assert.equal(outbound.toAddress, "c@out.com");
	assert.equal(outbound.subject, "Re: Question");

	// Both ends must share a grouping key. MailKite only sets threadId once a
	// conversation exists, so the first inbound message has none — the reply adopts
	// its messageId and the original is backfilled to match.
	const inbound = after.messages.find((m: any) => m.direction !== "outbound");
	assert.equal(outbound.threadId, "m_thread");
	assert.equal(inbound.threadId, "m_thread");
});
